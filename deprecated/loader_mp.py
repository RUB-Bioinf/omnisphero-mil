...
class LoaderProcess(multiprocessing.Process):
    ...
    def __init__(self,
                 # Unique params for the loader
                 filepath: str, worker_verbose: bool, normalize_enum: int, label_0_well_indices: [int],
                 label_1_well_indices: [int], constraints_0: [int], constraints_1: [int],
                 channel_inclusions: [bool], unrestricted_experiments_override: [str], requires_unzip: bool,
                 include_raw: bool = True, daemon: bool = True, name: str = None

                 # Original 'multiprocessing.Process' Parameters
                 # group: None = ..., target: Optional[Callable[..., Any]] = ..., name: Optional[str] = ...,
                 # args: Tuple[Any, ...] = ..., kwargs: Mapping[str, Any] = ..., *, daemon: Optional[bool] = ...
                 ) -> None:
        super().__init__(daemon=daemon, name=name)

        # Setting argument fields
        self.filepath = filepath
        self.worker_verbose = worker_verbose
        self.normalize_enum = normalize_enum
        self.label_0_well_indices = label_0_well_indices
        self.label_1_well_indices = label_1_well_indices
        self.constraints_0 = constraints_0
        self.constraints_1 = constraints_1
        self.channel_inclusions = channel_inclusions
        self.unrestricted_experiments_override = unrestricted_experiments_override
        self.include_raw = include_raw
        self.requires_unzip = requires_unzip

        # Setting other fields
        self._loading_finished = multiprocessing.Value('b', False)
        self._loading_exception: Exception = None
        self._loading_results = None

    def is_finished(self):
        return self._loading_finished.value

    def get_loading_exception(self):
        assert self.is_finished()
        return self._loading_exception

    def get_loading_results(self):
        assert self.is_finished()
        return self._loading_results

    def run(self):
        # Reading and loading JSON data

        # One large try catch, so nothing will be soft-locked
        try:
            assert os.path.exists(self.filepath)
            if self.requires_unzip:
                X, y_bag, y_tiles, X_raw, X_metadata, bag_name, experiment_name, well_name = unzip_and_read_JSON(
                    filepath=self.filepath,
                    worker_verbose=self.worker_verbose,
                    normalize_enum=self.normalize_enum,
                    label_0_well_indices=self.label_0_well_indices,
                    label_1_well_indices=self.label_1_well_indices,
                    constraints_0=self.constraints_0,
                    constraints_1=self.constraints_1,
                    channel_inclusions=self.channel_inclusions,
                    unrestricted_experiments_override=self.unrestricted_experiments_override,
                    include_raw=self.include_raw
                )
            else:
                X, y_bag, y_tiles, X_raw, X_metadata, bag_name, experiment_name, well_name = read_JSON_file(
                    filepath=self.filepath,
                    worker_verbose=self.worker_verbose,
                    normalize_enum=self.normalize_enum,
                    label_0_well_indices=self.label_0_well_indices,
            ...
            # TODO implement a multi processing manager
            self._loading_results = (X, y_bag, y_tiles, X_raw, X_metadata, bag_name, experiment_name, well_name)
        except Exception as e:
            # In case there is an error, we can store it
            self._loading_exception = e
        finally:
            # lastly, marks this as done
            self._loading_finished.value = True
...
def load_bags_json(source_dir: str, max_workers: int, normalize_enum: int, label_0_well_indices: [int],
    ...
    for i in range(len(files)):
        ...
        if file.lower().endswith('.json.zip') or file.lower().endswith('.json'):
            print('')
            line_print(str(i) + '/' + str(len(files)) + ': Setting up a Process for: ' + filepath)

            task_list.append(LoaderProcess(
                # Specific file to file params
                requires_unzip=file.lower().endswith('.json.zip'),

                # Process params
                daemon=True,
                name='Loader Process for: ' + filepath,
    ...
    for t in task_list:
        t.start()
    print('')
    log.write('Started ' + str(len(task_list)) + ' tasks. Awaiting termination.')
    ...
    error_list = []
    print('')
    log.write('Multi-processed execution resulted in ' + str(len(task_list)) + ' tasks. Joining them now.')
    print('\n')
    ...
    for i in range(len(task_list)):
        task = task_list[i]
        line_print('Extracting task: ' + str(i + 1) + '/' + str(len(task_list)), include_in_log=False)

        e = task.get_loading_exception()
        if e is None:
            X_f, y_f, y_f_tiles, x_f_raw, x_f_metadata_raw, bag_name, experiment_name, well_name = task.get_loading_results()
            if X_f is not None and y_f is not None and y_f_tiles is not None and x_f_raw is not None and len(
                    X_f) != 0:
                X.append(X_f)
                y.append(y_f)
                ...
                log.write('Assumed experiment name (Task #' + str(
                    i) + '):\n' + experiment_name + ' - ' + well_name + ' <- ' + source_dir,
                          print_to_console=False, include_in_files=True)
        else:
            log.write('\n' + gct() + 'Error occurred in a task: ' + str(e) + '\n')
            tb = traceback.TracebackException.from_exception(e)
            for line in tb.stack:
                log.write(str(line))
            error_list.append(e)
    ...
    log.write('Wells with label 0 from ' + experiment_name + ': ' + str(bag_count_negative) + ' / ' + str(task_count))
    log.write('Wells with label 1 from ' + experiment_name + ': ' + str(bag_count_positive) + ' / ' + str(task_count))
...
def unzip_and_read_JSON(filepath: str, worker_verbose: bool, normalize_enum: int, label_0_well_indices: [int],
                        label_1_well_indices: [int], constraints_0: [int], constraints_1: [int],
